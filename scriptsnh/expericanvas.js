...
// Check the distance between passed Variable and this one.
function getDistanceBool(position1, position2, tempGap) {
...
function createBubble(initialPos) {
    ...
    var linePath = new paper.Path.Line(
            new paper.Point(0, 0),
            new paper.Point(0, 0));
        linePath.strokeColor = bubbleColour;//'black';
        linePath.strokeWidth = '20.0';
    ...
    function moveBubble() {
        ...
        // linePath.segments[0].position = bubblePos;
    ...
    function drawLine(point1, point2) {
        linePath.strokeColor = bubbleColour;
        linePath.segments[0].point = point1;
        linePath.segments[1].point = point2;
    }

    function hideLine() {
        linePath.strokeColor = null;
    }
       ...
       for(var i=0; i <= bubbles.length-1; i ++) {
            ...
            for(var j=0; j <= bubbles.length-1; j ++) {
                if(j!=i) {
                    ...
                    var testDistBool = getDistanceBool(bubble2Pos, bubble1Pos, bubGap);
                    if(testDistBool) {
                        bubble2.drawLine(bubble1Pos, bubble2Pos);
                        tempBool = false;
                    }
                }
            }
