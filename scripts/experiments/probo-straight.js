...
function makeRandomPointLocal(_point) {
  ...
  var randomDirection = randomDirArray[Math.floor(Math.random()*randomDirArray.length)];
  // var randomDirection = new Point(getRandomA(-1, 1),getRandomA(-1, 1));
  // var randomDirection = new Point(1,1);
  var randomPoint = lastPoint + (randomDirection.normalize() * (drawDistance + 1));
  return randomPoint;
}
...
function defineStyle(){
  ...
  // var randomHue = 30;//getRandomA(0, 100);
